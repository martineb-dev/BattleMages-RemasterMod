...
if (-not $ZipPath) {
    ...
    $dialog.Title = 'Select BM_Paladin_Realism_GameTest_03.zip (shoulder clearance fix)'
    $dialog.Filter = 'Realism GameTest 03|BM_Paladin_Realism_GameTest_03*.zip|ZIP archives|*.zip'
    $dialog.FileName = 'BM_Paladin_Realism_GameTest_03.zip'
    try {
...
$expectedHash = '6489f91f0e98a4778697d1461a2f422cb7a43de383b476db7a67b6bce6060589'
if ($actualHash -ne $expectedHash) { throw "Select the NEW BM_Paladin_Realism_GameTest_03.zip. Selected SHA256: $actualHash" }
& (Join-Path $PSScriptRoot 'Test-SourceBaseline.ps1')
$run = Join-Path (Join-Path $work 'reports') ('realism-03-' + [DateTime]::UtcNow.ToString('yyyyMMdd-HHmmss') + '-' + [guid]::NewGuid().ToString('N'))
...
try {
    ...
    $package = Join-Path $builds ('paladin-realism-game-03-' + [guid]::NewGuid().ToString('N'))
    Expand-Archive -LiteralPath $ZipPath -DestinationPath $package
    $manifest = Get-Content -LiteralPath (Join-Path $package 'manifest.json') -Raw | ConvertFrom-Json
    if ($manifest.build -ne 'cm1-comparison-paladin-realism-game-03') { throw 'Unexpected build identifier.' }
    $phase = 'install'
    ...
    if (-not $NoLaunch) {
        ...
        $phase = 'launch'
        Write-Host 'Start a NEW Part I / Chapter 1: Final Examination. Inspect both shoulders from behind while walking and attacking.' -ForegroundColor Cyan
    ...
    Write-BMJson ([ordered]@{ build='cm1-comparison-paladin-realism-game-03'; phase=$phase;
